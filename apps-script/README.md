...
It is wired into `otp-a` (see `createConfiguredVerificationProviders`), so the
orchestrator sees an ordinary OTP provider and rotates to `otp-b`/`otp-c` if one
is ever configured.
...
```sh
npx wrangler secret put OTP_A_PROVIDER_APPS_SCRIPT_URL   # the /exec URL
npx wrangler secret put OTP_A_PROVIDER_SHARED_SECRET     # same secret as SHARED_SECRET
```

`OTP_A_DAILY_QUOTA` is an optional non-secret var (defaults to the value the
