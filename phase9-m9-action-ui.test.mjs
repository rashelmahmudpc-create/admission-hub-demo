...
function build({ lang = 'bn', msgs = [], fetchImpl = null } = {}) {
  ...
  const fn = new Function('esc', 'T', 'lang', 'window', 'fetch', 'msgs', 'save', 'renderMsgs', 'toast',
    block + '\nreturn { ACT_ACTIONS, actLabel, actionCardHtml, actionExpired, dropExpiredAction, decideAction, actParseIntent, offerAction };');
...
t('M9UI-১. "উত্তর ছোট করে দাও" → prefs.write{responseLen:short}',
  (() => { const i = build().actParseIntent('উত্তর ছোট করে দাও'); return i && i.action === 'prefs.write' && i.args.responseLen === 'short'; })());
t('M9UI-২. "keep it concise" and "answer in English" map to length and language',
...
t('M9UI-৩. a plain study question proposes nothing at all',
  (['বাংলা সন্ধি বুঝিয়ে দাও', 'give me 5 MCQs', 'আমার পারফরম্যান্স কেমন?', '']
    .every((q) => build().actParseIntent(q) === null)));
t('M9UI-৪. every parsed arg key is a declared prefs.write argument',
  (() => {
...
t('M9UI-৫. the pending card renders the proposal summary and both choices',
  (() => {
...
t('M9UI-৬. a long/terminal state renders a status line instead of buttons',
  (() => {
...
t('M9UI-৭. the card is escape-first — a hostile summary cannot inject markup',
  (() => {
...
t('M9UI-৮. "না" resolves the card locally and never calls confirm',
  (async () => {
...
t('M9UI-৯. "হ্যাঁ" POSTs only {token} to /api/ai/actions/confirm and marks applied',
  (async () => {
...
t('M9UI-১০. a refused confirm (expired) becomes a terminal, non-retryable state',
  (async () => {
    const m = { pending: true, actId: 'tok', actAction: 'prefs.write', actExpiresAt: Date.now() + 60000 };
...
t('M9UI-১১. a network failure keeps the card pending and warns offline (no false success)',
  (async () => {
    const m = { pending: true, actId: 'tok', actAction: 'prefs.write', actExpiresAt: Date.now() + 60000 };
...
t('M9UI-১২. an expired proposal is refused locally without any round-trip',
  (async () => {
...
t('M9UI-১৩. a double tap cannot fire two confirms',
  (async () => {
...
t('M9UI-১৪. a successful propose pushes one pending card carrying the server id',
  (async () => {
    const msgs = [];
    ...
    return ok === true && card.pending === true && card.actId === 'srv-1' && card.actSummary === 'নতুন সেটিং' && msgs.length === 1;
  })());
t('M9UI-১৫. when the layer is off (403 layer-disabled) the chat falls through to a normal answer',
  (async () => {
    const msgs = [];
...
t('M9UI-১৬. the client asks for a proposal — it never writes preferences itself',
...
t('M9UI-১৭. the UI strings for both languages exist',
...
t('M9UI-১৮. the confirm route still requires a signed-in student and a single-use token',
